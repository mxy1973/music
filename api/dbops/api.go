...
func GetUser(loginName string) (*defs.User, error) {
	...
	if err != nil {
		log.Printf("select mysql data error: %s", err)
		return nil, err
	}
...
func ListVideoInfo(uname string, from, to int) ([]*defs.VideoInfo, error)  {
	...
	if err != nil {
		log.Printf("mysql data ListVideoInfo yufa error: %s", err)
		return nil, err
	}
	var res []*defs.VideoInfo
	rows, err := stmtOut.Query(uname, from, to)
	if err != nil {
		log.Printf("mysql data ListVideo error: %s", err)
		return res, err
	}
	...
	for rows.Next() {
		...
		vi := &defs.VideoInfo{Id:id, Name:name, AuthorId:aid, DisplayCtime:ctime}
		res = append(res, vi)
	}
	defer stmtOut.Close()
	return res, nil
